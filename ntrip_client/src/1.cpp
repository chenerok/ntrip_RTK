...
// NTRIP服务器信息
 ...
 std::string username = "qxcorsh0077791";
 std::string password = "cc52aa7";
 char* GNGGA_data="$GNGGA,063244.40,3859.14005,N,11720.17593,E,1,12,0.64,25.9,M,-5.1,M,,*6C\r\n";


int init_serialport_recv(serial::Serial& port,int baudrate,char* port_name);
int init_serialport_send(serial_port& port,int baudrate,char* port_send_name);
std::string StringToBase64(const std::string& input) ;
void RecePro(std::string s , double& lat , double& lon, double& high );
std::string get_GPS_data(std::string& strRece,serial::Serial& port);
...
sensor_msgs::NavSatFix gga_data;
...
int main(int argc,char** argv) 
{
    ...
    ros::NodeHandle nh;
    //订阅话题
    //gga_sub=nh.subscribe<sensor_msgs::NavSatFix>("/ublox_driver/receiver_lla",100,get_gga);
    // 初始化串口
    io_service io;
    serial_port port_send(io);
    init_serialport_send(port_send,115200,"/dev/ttyUSB0");
    ...
    while (ros::ok())  
    {
        i++;
        std::string raw,gga1;
        gga1=get_GPS_data(raw,port_recv);
        if(gga1.size()>90)
            continue;
        if(i%10==0)
        {
            ...
            write(port_send,buffer(buffer1));
        }
       loop_rate.sleep();
    }
            ...
            while ( i < strRece.length() )
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        std::cout<<gga_data<<gga_data.size()<<std::endl;
                        //保留位数7位
